...
func Distributing(king database.Player, players []database.Player, roundNumber, victoryRate int32, mode int32, additional bool) (collections []map[database.Player][]string) {
	...
	for _, selected := range buildDistribution(roundNumber, victoryRate) {
		...
		for _, pokers := range Acquire5(len(players)) {
			b, w, _, _, err := SearchBestPokerPattern(pokers, mode, additional)
			if err != nil {
				panic(err)
			}
